  ...
  return (
    <form action={formAction} className="max-w-xl space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <div className="space-y-1">
          <label htmlFor="point_cost" className="text-sm font-medium">Biaya poin *</label>
          <input id="point_cost" name="point_cost" type="number" min={1} required defaultValue={v("point_cost") === "" ? 500 : v("point_cost")}
            className="w-full rounded border bg-white px-3 py-2 text-sm" />
        </div>
        <div className="space-y-1">
          <label htmlFor="demo_value" className="text-sm font-medium">Nilai demo</label>
      ...
      <button type="submit" disabled={pending}
        ...
        {pending ? "Menyimpan..." : mode === "create" ? "Buat reward" : "Simpan"}
      </button>
    </form>
  );
}
